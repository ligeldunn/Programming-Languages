...
(define (eval-statement env statement [slow? #f])
  ...
  (match statement
    ...
    ;; Line expression
    [(line-expr x y x1 y1)
     ;; store xcoord and ycoord values
    (let ([xcoord (eval-expr env x)]
          [ycoord (eval-expr env y)]
          [xcoord1 (eval-expr env x1)]
          [ycoord1 (eval-expr env y1)])
      ;; draw line
      (draw-line xcoord ycoord xcoord1 ycoord1))]
    ...
    [(assignment-expr (var-expr var)  val)
     ;; store search var and searchval
     (let ([searchvar (apply-env env var)]
           [searchval (eval-expr env val)])
       ;; if searchvar exists in env
       (if searchvar
           (setref! (deref searchvar) searchval)
           ;; extend env (create it)
           (extend-env env var searchval)))]
    ...
    [(command-fun sym params body)
     ;; create closure using current env
     (let ([close (closure sym params body env)])
           ;; extend env and pass on the newly extended env
           (let ([newenv (extend-env env sym close)])newenv))]
    ...
    ;;; TODO: application as statements
    [(apply-expr sym exprs)
     ;; Look up function def to get its closure
     (let ([funcdef (apply-env env sym)])
       ;; Evalulate the arguments and call the function 
       (eval-expr env (closure-params (deref funcdef)))
       (funcdef)
...
(define (eval-expr env expr)
  (match expr
    ...
    [(apply-expr sym exprs)
     ;; Get function Def from curent env
     (let ([funcdef (apply-env env sym)])
       ;; Evaluate arguments and extend 
       (eval-expr (closure-body (deref funcdef))
                  (extend-env (closure-params (deref funcdef)) (eval-expr env exprs)
                               ;; Evaluate actual parameter before binding and return result
                               (closure-env funcdef)))funcdef)]
